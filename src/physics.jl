...
function advection(c, wd, dz)
    ...
    flux_up = positive_wd[2:end,:].*c[3:end-2,:] + negative_wd[2:end,:].*c[4:end-1,:]
    flux_down = positive_wd[1:end-1,:].*c[2:end-3,:] + negative_wd[1:end-1,:].*c[3:end-2,:]
    adv = (flux_up .- flux_down)./dz 
    return adv 
end
...
function diffusion(c, kappa_z, dz)
    ...
    flux_up = kappa_z[1:kappa_z_len-1] .* (c[2:kappa_z_len,:] .- c[1:kappa_z_len-1,:]) ./dz 
    flux_down = kappa_z[2:kappa_z_len] .* (c[3:kappa_z_len+1,:] .- c[2:kappa_z_len,:]) ./dz
    diff = (flux_down .- flux_up) ./ dz
    return diff
end
